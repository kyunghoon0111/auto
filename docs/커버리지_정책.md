...
새 도메인을 추가하려면 (예: 새 데이터 소스):

1. `config/schema.yaml`을 통해 CORE 스키마에 테이블 추가.
2. `config/policies/coverage_policy.yaml`에 도메인 추가:
...
domains:
  new_data_source:
    close_period_enforcement: true
    tables:
      - table: fact_new_source
        requirement: REQUIRED
        description: "새 소스 시스템의 데이터"
```

3. `python run.py --init`을 실행하여 새 테이블 생성.
4. 커버리지 프레임워크가 새 도메인을 모든 검사에 자동으로 포함.
